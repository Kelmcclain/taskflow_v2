...
import { Calendar, ChevronRight, Folder, LayoutGrid, List, Loader2, Plus, Search, Users } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  ...
  const WorkspaceCard: React.FC<{ workspace: WorkspaceWithMembers }> = ({ workspace }) => (
    <Link
    ...
    >
      <div className="p-6">
        ...
        {workspace.description && (
          <p className="text-gray-600 dark:text-gray-400 text-sm mb-4 line-clamp-2">{workspace.description}</p>
        )}
        <div className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
          <div className="flex items-center">
            <Users className="h-4 w-4 mr-1.5" />
            {(workspace.workspace_members[0]?.count || 1)} member{(workspace.workspace_members[0]?.count || 1) !== 1 ? 's' : ''}
          </div>
  ...
  const WorkspaceRow: React.FC<{ workspace: WorkspaceWithMembers }> = ({ workspace }) => (
    <Link
      to={`/workspace/${workspace.id}`}
    ...
    >
      <div className="flex items-center flex-1 min-w-0">
        ...
        <div className="ml-4 flex-1 min-w-0">
          ...
          {workspace.description && (
            <p className="text-sm text-gray-600 dark:text-gray-400 truncate">{workspace.description}</p>
          )}
        </div>
      </div>
